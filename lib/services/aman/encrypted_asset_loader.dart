...
Future<void> pullLatestEncryptedAsset() async {
  const url =
      'https://raw.githubusercontent.com/Rabie-Hood/rdo/main/assets/lst_rdo.aes';
  try {
...
Future<String> decryptJson() async {
  ...
  final cipher = Uint8List.fromList(data.sublist(12));
  final key    = Key.fromUtf8(_obfKey());
  final enc    = Encrypter(AES(key, mode: AESMode.gcm));

  final plain = enc.decryptBytes(Encrypted(cipher), iv: iv);
